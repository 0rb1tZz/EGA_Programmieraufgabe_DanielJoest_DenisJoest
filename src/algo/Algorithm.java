...
public abstract class Algorithm<T extends Comparable<T>, V> {
    ...
    private volatile Map<Integer, Node1RenderState> finishedStateCache = null;
    ...
    public Algorithm(Graph1<T> graph) {
        this.graph = graph;
    }
     ...
     * @param node the node
     */
    public void draw(Graphics2D g2d, Node1<T> node) {
        g2d.drawString("N/A", 0, 0);
    }

    // --- New API for rendering/querying node state ---
    public enum Node1RenderState {
        DEFAULT,
        OPEN,
        CLOSED,
        PATH,
     ...
     * base class will build a snapshot of these states and further calls to {@link #getNode1RenderState(Node1)}
     * will be served from that snapshot for efficiency.
     */
    protected Node1RenderState nodeRenderState(Node1<T> node) {
        return Node1RenderState.DEFAULT;
    }
     ...
     * {@link #nodeRenderState(Node1)} instead of this method.
     */
    public final Node1RenderState getNode1RenderState(Node1<T> node) {
        var cache = finishedStateCache;
        if (cache != null) {
            return cache.getOrDefault(node.getId(), Node1RenderState.DEFAULT);
        }
     ...
     */
    public Iterable<Node1<T>> getInterestingNode1s() {
        return null;
    }
    ...
    private void buildFinishedStateCache() {
        try {
            Map<Integer, Node1RenderState> map = new HashMap<>();
            for (Node1<T> n : graph.getAllNodes()) {
                Node1RenderState s = nodeRenderState(n);
                map.put(n.getId(), s == null ? Node1RenderState.DEFAULT : s);
            }
